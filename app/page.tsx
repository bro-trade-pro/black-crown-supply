...
function Catalog({title,subtitle,id,products}:{title:string;subtitle:string;id:string;products:Product[]}){return <section className="catalog" id={id}><div className="sectionHead"><div><p>{subtitle}</p><h2>{title}</h2></div><span>{products.length} références</span></div><div className="grid">{products.map(p=>{const img=imageUrl(p.image_url);return <article className="card" key={p.id}>{img?<img src={img} alt={p.nom}/>:<div className="placeholder">BLACK CROWN</div>}<div className="cardBody">{p.marque&&<small>{p.marque}</small>}<h3>{p.nom}</h3>{p.contenance&&<p>{p.contenance}</p>}<div className="price">{(p.prix_ttc_cents/100).toFixed(2).replace(".",",")} € <span>TTC</span></div><a className="productButton" href={`/produit/${p.id}`}>Voir le produit</a></div></article>})}</div></section>}
